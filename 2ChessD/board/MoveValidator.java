    ...
    */
    ...
    public boolean isCheckmate(boolean color) {
        ...
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if (this.board.getPlayBoard()[i][j] != null
                        && this.board.getPlayBoard()[i][j].getColor() == color) {               //if the tile has a piece and the color of the piece is the same as the color of the king
                    for (int k = 0; k < 8; k++) {
                        for (int l = 0; l < 8; l++) {                            
                            if (this.isValidMove(i, j, k, l)                //if the move is valid and the king is still in check by the same piece  
                                && this.checkedBy.equals(checkedByTemp)
                                && this.removesCheck(i, j, k, l)) {                                               
                                System.out.println(this.board.getPlayBoard()[i][j].getPiece().getName());
